...
<div id="app" class="default-box" >
    ...
    <template v-if="showEntrance">
        <div class="pos-top ">
            <div class="center" style="background: none">
                ...
                <div class="pos-top-r">
                    <input type="button" value="打开" class="hid">
                    <a :href="urlEntrance" ><div>打开</div></a>
        ...
        <div class="source-box clear" :style='{"padding-top": showEntrance? "50px" : "10px"}'>
            <div class="top left">
                ...
                <ul class="left">
                    ...
                    <li class="original"><span v-text="isOriginal == 0 ? '原创':'来源：'"></span>&nbsp;<span v-text="forwardFrom"></span></li>
                </ul>
                <div class="original right">
                    <template v-if="ver == 2 && viewerId && circle_isFocused != 2">
                        <template v-if="circle_isFocused == 0">
                            <span class="focus-before" @click="focusCircle">+关注</span>
                        </template>
                        <template v-else>
                            <span >已关注</span>
                        </template>
                    ...
                    <template v-if="ver == 2  && viewerId && isOriginal">
                        <a :href="certPage"><span class="article-cert">文章证书</span></a>
                    </template>
                </div>
            </div>
        </div>
    ...
    <ul class="operation-list clear" :class='{"hid": replyChildStatus}'>
        <li class="view">阅读&nbsp;<span v-text="readCount"></span></li>
    ...
    <div class="comment-list">
        <div class="comment-box clear" :class='{"hid": replyChildStatus}'>
            <div class="comment-top clear">
                <span class="left comment-number">评论 <span v-text="replyCount"></span></span>
                <span v-if="viewerId && !ownArticle" class="right write-comment" @click="doReply">写评论</span>
            </div>
        </div>
...
<script type="text/x-template" id="replyInput">
    <div>
        <div class="write">
            <textarea placeholder="评论对所有人可见" v-model="replyInput" ref="replyDom"></textarea>
...
<script data-main="../resources/js/articleMain_zh" src="../resources/js/require.js"></script>
