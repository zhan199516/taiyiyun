...
<div id="app" class="default-box" >
    ...
    <template v-if="showEntrance">
        <div class="pos-top ">
            <div class="center" style="background: none">
                ...
                <div class="pos-top-r">
                    <input type="button" value="Open" class="hid">
                    <a :href="urlEntrance" ><div>Open</div></a>
        ...
        <div class="source-box clear" :style='{"padding-top": showEntrance? "50px" : "10px"}'>
            <div class="top left">
                ...
                <ul class="left">
                    ...
                    <li class="original"><span v-text="isOriginal == 0 ? 'Original':'Reprinted'"></span>&nbsp;<span v-text="forwardFrom"></span></li>
                </ul>
                <div class="original right">
                    <template v-if="ver == 2 && viewerId && circle_isFocused != 2">
                        <template v-if="circle_isFocused == 0">
                            <span class="focus-before" @click="focusCircle">+Attention</span>
                        </template>
                        <template v-else>
                            <span >Has been concerned</span>
                        </template>
                    </template>
                    <template v-if="ver == 2  && viewerId && isOriginal">
                        <a :href="certPage"><span class="article-cert">Article certificate</span></a>
                    </template>
                </div>
            </div>
        </div>
    ...
    <div class="cover" :style="{display: alertConfirm}">
        <div class="tip-box" :class="{animate: alertConfirm != 'none'}">
            <h6>Attention</h6>
            <p>When the comment is deleted, it will not appear in the comment list</p>
            <div class="button-box">
                <button class="cancel-btn" @click="confirmCancel">Cancel</button>
                <button class="delete-btn" @click="confirmYes">Delete</button>
            </div>
        </div>
    </div>
    ...
    <ul class="operation-list clear" :class='{"hid": replyChildStatus}'>
        <li class="view">Read&nbsp;<span v-text="readCount"></span></li>
        <li class="share">Share&nbsp;<span v-text="forwardCount"></span></li>
    ...
    <div class="comment-list">
        <div class="comment-box clear" :class='{"hid": replyChildStatus}'>
            <div class="comment-top clear">
                <span class="left comment-number">Comment <span v-text="replyCount"></span></span>
                <span v-if="viewerId && !ownArticle" class="right write-comment" @click="doReply">Review</span>
            </div>
        </div>
        ...
        <div class="comment-box pad-top10 clear">
            ...
            <div class="comment-content left" :style="{width: commentWidth}">
                ...
                <div class="write-content clear">
                    ...
                    <div>
                        <span class="right delete" v-if="canDelete" @click="deleteMe">Delete</span>
                    </div>
                </div>
                <div>
                ...
                <div><span class="reply-btn right" v-if="canReply" @click="reply">Reply</span></div>
                <div></div>
            </div>
        </div>
    </div>
...
<script type="text/x-template" id="replyInput">
    <div>
        <div class="write">
            <textarea placeholder="Comments are visible to everyone" v-model="replyInput" ref="replyDom"></textarea>
            <button @click="send">Send</button>
            <button class="back write-back" @click="cancel">Return</button>
        </div>
    </div>
</script>
...
<script data-main="../resources/js/articleMain_en" src="../resources/js/require.js"></script>
