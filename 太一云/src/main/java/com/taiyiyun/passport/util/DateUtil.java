...
public final class DateUtil {
	...
	private static final Map<String, DateFormat> FORMAT_MAP = new HashMap<>();
	/**
	 * 要用到的DATE Format的定义
	 */
	public static SimpleDateFormat sdfDateTime = new SimpleDateFormat(Format.DATE_TIME_HYPHEN.getValue());
	...
	public enum Format {
		DATE_MONTH("yyyyMM"), // 6
		DATE("yyyyMMdd"), // 8
		DATE_MINUTE("yyyyMMddHHmm"), // 12
		DATE_TIME("yyyyMMddHHmmss"), // 14
		DATE_MONTH_HYPHEN("yyyy-MM"), // 7
		DATE_HYPHEN("yyyy-MM-dd"), // 10
		DATE_MINUTE_HYPHEN("yyyy-MM-dd HH:mm"), // 16
		DATE_TIME_HYPHEN("yyyy-MM-dd HH:mm:ss"), // 19
		DATE_TIME_MONTH("MM-dd HH:mm:ss"), // 19
		DATE_MONTH_VIRGULE("yyyy/MM"), // 7
		DATE_VIRGULE("yyyy/MM/dd"), // 10
		DATE_MINUTE_VIRGULE("yyyy/MM/dd HH:mm"), // 16
		DATE_TIME_VIRGULE("yyyy/MM/dd HH:mm:ss"), // 19
		TIME_STAMP_10("010d%"), // 10
		TIME_STAMP_13("013d%"),
		NONE("[NONE]"); // 13
	...
	public static String toString(Date date, Format format) {
		...
		default:
			return toString(date, format.value);
		}
	}
	
	public static String toString(Date date, String pattern) {
		if (!FORMAT_MAP.containsKey(pattern)) {
			FORMAT_MAP.put(pattern, new SimpleDateFormat(pattern));
		}
		return FORMAT_MAP.get(pattern).format(date);
	}
	
	public static Date fromString(String dateString) {
		dateString = new String(dateString.trim().replaceAll("  ", " "));
		Format format = getFormat(dateString);
		switch (format) {
		case NONE:
			return null;
		case TIME_STAMP_10:
		...
		default:
			return fromString(dateString, format.value);
		}
	}
	...
	public static Date fromString(String dateString, String pattern) {
		...
		try {
			return FORMAT_MAP.get(pattern).parse(dateString);
	...
	private static Format getFormat(String dateString) {
		int len = dateString.length();
		switch (len) {
		case 6:
			return NumberUtil.isPositiveInteger(dateString) ? Format.DATE_MONTH : Format.NONE;
		case 7:
			return hasVirgule(dateString) ? Format.DATE_MONTH_VIRGULE : hasHyphen(dateString) ? Format.DATE_MONTH_HYPHEN : Format.NONE;
		case 8:
			return NumberUtil.isPositiveInteger(dateString) ? Format.DATE : Format.NONE;
		case 10:
			return hasVirgule(dateString) ? Format.DATE_VIRGULE : hasHyphen(dateString) ? Format.DATE_HYPHEN : NumberUtil.isPositiveInteger(dateString) ? Format.TIME_STAMP_10 : Format.NONE;
		case 12:
			return Format.DATE_MINUTE;
		case 13:
			return NumberUtil.isPositiveInteger(dateString) ? Format.TIME_STAMP_13 : Format.NONE;
		case 14:
			return NumberUtil.isPositiveInteger(dateString) ? Format.DATE_TIME : Format.NONE;
		case 16:
			return hasVirgule(dateString) ? Format.DATE_MINUTE_VIRGULE : hasHyphen(dateString) ? Format.DATE_MINUTE_HYPHEN : Format.NONE;
		case 19:
			return hasVirgule(dateString) ? Format.DATE_TIME_VIRGULE : hasHyphen(dateString) ? Format.DATE_TIME_HYPHEN : Format.NONE;
		default:
			return Format.NONE;
		}
	}
	
	private static boolean hasVirgule(String str) {
		return str.indexOf("/") >= 0;
	}
	
	private static boolean hasHyphen(String str) {
		return str.indexOf("-") >= 0;
	}
	...
	public static boolean isExpired(Date date1, Date date2) {
		...
		if (result == 1) {
			System.out.println("................红包已经过期......result......." + result);
			return true;
		} else {
			return false;
		}
	...
	public static java.sql.Timestamp toSqlTimestamp(String sDate) {
		...
		if (sDate.length() != Format.DATE_HYPHEN.getValue().length()
				&&sDate.length() != Format.DATE_TIME_HYPHEN.getValue().length()) {
			return null;
		}
		return toSqlTimestamp(sDate,
				sDate.length() == Format.DATE_HYPHEN.getValue().length()
						?Format.DATE_HYPHEN.getValue()
						:Format.DATE_TIME_HYPHEN.getValue());
	...
	public static java.sql.Timestamp toSqlTimestamp(String sDate, String sFmt) {
		...
		if (sFmt.equals(Format.DATE_TIME_HYPHEN.getValue())) {
			temp = sDate.replace('/', '-');
			temp = temp + ".000000000";
		} else if (sFmt.equals(Format.DATE_HYPHEN.getValue())) {
			temp = sDate.replace('/', '-');
	...
	public static String toSqlTimestampString2(java.sql.Timestamp dt) {
		...
		String temp = toSqlTimestampString(dt, Format.DATE_TIME_HYPHEN.getValue());
	...
	public static String toSqlTimestampString(java.sql.Timestamp dt, String sFmt) {
		...
		temp = dt.toString();
		if (sFmt.equals(Format.DATE_TIME_HYPHEN.getValue()) || // "YYYY/MM/DD
		// HH24:MI:SS"
				sFmt.equals(Format.DATE_HYPHEN.getValue())) { // YYYY/MM/DD
	...
	public static String timestampToStringYMD(java.sql.Timestamp timestamp) {
		SimpleDateFormat sdf = new SimpleDateFormat(Format.DATE_HYPHEN.getValue());
	...
	public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
		...
		Date endDate = null;
		try {
			startDate = DateUtil.fromString(startDateStr, Format.DATE_HYPHEN.getValue());
			endDate = DateUtil.fromString(endDateStr, Format.DATE_HYPHEN.getValue());
		} catch (Exception e) {
			logger.error(e.getMessage());
			return false;
		}
    ...
    */
	...
	public static void main(String[] args) {
		System.out.println(toString(new Date(), Format.TIME_STAMP_10));
		System.out.println(toString(new Date(), Format.TIME_STAMP_13));
		System.out.println(toString(new Date(), Format.DATE));
		System.out.println(toString(new Date(), Format.DATE_HYPHEN));
		System.out.println(toString(new Date(), Format.DATE_MONTH));
		System.out.println(toString(new Date(), Format.DATE_MONTH_HYPHEN));
		System.out.println(toString(new Date(), Format.DATE_TIME));
		System.out.println(toString(new Date(), Format.DATE_TIME_HYPHEN));
		System.out.println(toString(new Date(), Format.DATE_TIME_MONTH));


		System.out.println("===============================");
		System.out.println(fromString("1497354327"));
		System.out.println(fromString("1497354327151"));
		System.out.println(fromString("20170613"));
		System.out.println(fromString("2017-06-13"));
		System.out.println(fromString("201706"));
		System.out.println(fromString("2017-06"));
		System.out.println(fromString("20170613194527"));
		System.out.println(fromString("2017-06-13 19:45:27"));
		Date date = new Date();
		System.out.println(toString(date, "yyyy"));
		System.out.println(toString(date, "MM"));
		System.out.println(toString(date, "dd"));
		java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-M-d HH:mm:ss");
		for(int i=0;i<10000;i++) {
			System.out.println(System.currentTimeMillis() + "nano:"+System.nanoTime());
		}
	}
