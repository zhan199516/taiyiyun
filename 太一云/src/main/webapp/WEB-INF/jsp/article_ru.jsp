...
<div id="app" class="default-box" >
    ...
    <template v-if="showEntrance">
        <div class="pos-top ">
            <div class="center" style="background: none">
                ...
                <div class="pos-top-r">
                    <input type="button" value="открыть" class="hid">
                    <a :href="urlEntrance" ><div>открыть</div></a>
        ...
        <div class="source-box clear" :style='{"padding-top": showEntrance? "50px" : "10px"}'>
            <div class="top left">
                ...
                <ul class="left">
                    ...
                    <li class="original"><span v-text="isOriginal == 0 ? 'оригинал':'воспроизведенный'"></span>&nbsp;<span v-text="forwardFrom"></span></li>
                </ul>
                <div class="original right">
                    <template v-if="ver == 2 && viewerId && circle_isFocused != 2">
                        <template v-if="circle_isFocused == 0">
                            <span class="focus-before" @click="focusCircle">+внимание</span>
                        </template>
                        <template v-else>
                            <span >Был обеспокоен</span>
                        </template>
                    </template>
                    <template v-if="ver == 2  && viewerId && isOriginal">
                        <a :href="certPage"><span class="article-cert">Сертификат статьи</span></a>
                    </template>
                </div>
            </div>
        </div>
    ...
    <div class="cover" :style="{display: alertConfirm}">
        <div class="tip-box" :class="{animate: alertConfirm != 'none'}">
            <h6>подсказки</h6>
            <p>Это  доказывает, что только для технических испытаний и подтверждения концепции, не должны использоваться для любых других целей</p>
            <div class="button-box">
                <button class="cancel-btn" @click="confirmCancel">отменить</button>
                <button class="delete-btn" @click="confirmYes">Удалить</button>
            </div>
        </div>
    </div>
    ...
    <ul class="operation-list clear" :class='{"hid": replyChildStatus}'>
        <li class="view">чтение&nbsp;<span v-text="readCount"></span></li>
        <li class="share">поделиться&nbsp;<span v-text="forwardCount"></span></li>
    ...
    <div class="comment-list">
        <div class="comment-box clear" :class='{"hid": replyChildStatus}'>
            <div class="comment-top clear">
                <span class="left comment-number">Замечания <span v-text="replyCount"></span></span>
                <span v-if="viewerId && !ownArticle" class="right write-comment" @click="doReply">написать  комментарий</span>
            </div>
        </div>
        ...
        <div class="comment-box pad-top10 clear">
            ...
            <div class="comment-content left" :style="{width: commentWidth}">
                ...
                <div class="write-content clear">
                    ...
                    <div>
                        <span class="right delete" v-if="canDelete" @click="deleteMe">Удалить</span>
                    </div>
                </div>
                <div>
                ...
                <div><span class="reply-btn right" v-if="canReply" @click="reply">ответ</span></div>
                <div></div>
            </div>
        </div>
    </div>
...
<script type="text/x-template" id="replyInput">
    <div>
        <div class="write">
            <textarea placeholder="видимый для всех комментариев" v-model="replyInput" ref="replyDom"></textarea>
            <button @click="send">отправить</button>
            <button class="back write-back" @click="cancel">возвращение</button>
        </div>
    </div>
</script>
...
<script data-main="../resources/js/articleMain_ru" src="../resources/js/require.js"></script>
