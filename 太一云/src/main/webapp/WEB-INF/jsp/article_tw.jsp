...
<div id="app" class="default-box" >
    ...
    <template v-if="showEntrance">
        <div class="pos-top ">
            <div class="center" style="background: none">
                ...
                <div class="pos-top-r">
                    <input type="button" value="打開" class="hid">
                    <a :href="urlEntrance" ><div>打開</div></a>
        ...
        <div class="source-box clear" :style='{"padding-top": showEntrance? "50px" : "10px"}'>
            <div class="top left">
                ...
                <ul class="left">
                    ...
                    <li class="original"><span v-text="isOriginal == 0 ? '原創':'轉載'"></span>&nbsp;<span v-text="forwardFrom"></span></li>
                </ul>
                <div class="original right">
                    <template v-if="ver == 2 && viewerId && circle_isFocused != 2">
                        <template v-if="circle_isFocused == 0">
                            <span class="focus-before" @click="focusCircle">+關注</span>
                        </template>
                        <template v-else>
                            <span >已關注</span>
                        </template>
                    ...
                    <template v-if="ver == 2  && viewerId && isOriginal">
                        <a :href="certPage"><span class="article-cert">文章證書</span></a>
                    </template>
                </div>
            </div>
        </div>
    ...
    <div class="cover" :style="{display: alertConfirm}">
        <div class="tip-box" :class="{animate: alertConfirm != 'none'}">
            <h6>提示</h6>
            <p>該評論删除後將不顯示在評論清單中</p>
            <div class="button-box">
                <button class="cancel-btn" @click="confirmCancel">取消</button>
                <button class="delete-btn" @click="confirmYes">删除</button>
            </div>
        </div>
    </div>
    ...
    <ul class="operation-list clear" :class='{"hid": replyChildStatus}'>
        <li class="view">閱讀&nbsp;<span v-text="readCount"></span></li>
        <li class="share">分享&nbsp;<span v-text="forwardCount"></span></li>
    ...
    <div class="comment-list">
        <div class="comment-box clear" :class='{"hid": replyChildStatus}'>
            <div class="comment-top clear">
                <span class="left comment-number">評論 <span v-text="replyCount"></span></span>
                <span v-if="viewerId && !ownArticle" class="right write-comment" @click="doReply">寫評論</span>
            </div>
        </div>
        ...
        <div class="comment-box pad-top10 clear">
            ...
            <div class="comment-content left" :style="{width: commentWidth}">
                ...
                <div class="write-content clear">
                    ...
                    <div>
                        <span class="right delete" v-if="canDelete" @click="deleteMe">删除</span>
                    </div>
                </div>
                <div>
                ...
                <div><span class="reply-btn right" v-if="canReply" @click="reply">回复</span></div>
                <div></div>
            </div>
        </div>
    </div>
...
<script type="text/x-template" id="replyInput">
    <div>
        <div class="write">
            <textarea placeholder="評論對所有人可見" v-model="replyInput" ref="replyDom"></textarea>
            <button @click="send">發送</button>
            <button class="back write-back" @click="cancel">返回</button>
        </div>
    </div>
</script>
...
<script data-main="../resources/js/articleMain_tw" src="../resources/js/require.js"></script>
